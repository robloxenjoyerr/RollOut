...
export default function Host() {
    ...
    function updateRoomConfig() {
        setRoomConfig({ roomName: roomNameRef.current?.value, privateRoom: privateRoomRef.current?.checked })

        console.log(roomNameRef.current?.value)
        console.log(privateRoomRef.current?.checked)
    }
        ...
        <Card className="text-black" alignItems="center" width="w-fit" gap="gap-5" height="h-fit" justifyContent="around" >
            ...
            <div className="flex flex-row gap-5 w-fit">
                <div className="flex flex-col gap-3 justify-center w-30">
                    <Input placeholder="Room Name" width="w-full" id="roomName" ref={roomNameRef} onChange={updateRoomConfig} />
                    <div className="flex flex-row h-fit w-full border-2 p-2 border-black/20 rounded-2xl">
                        ...
                        <Input type="checkbox" width="w-5" height="h-4" className="self-center" id="is-private" ref={privateRoomRef} onChange={updateRoomConfig} />
                    </div>
                </div>
                <div className="flex flex-col gap-3 h-full w-30 border-2 p-2 border-black/20 rounded-2xl">
                    ...
                    <select id="modeSelect" className="border-none border-black/20 rounded-2xl p-0 w-full h-full">
                        {Modes && Modes.map((m) => {
                            return(
